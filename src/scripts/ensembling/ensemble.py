...
class CBTMInference:
    ...
    def generate_text(self, 
        ...
        """
        # Tokenize initial context and move to device
        ...
        print(f"🎯 Starting generation with context: \"{context[:100]}...\"")
        
        for step in range(max_new_tokens):
            # Recompute ensemble weights periodically
            ...
            # Update context for weight recomputation (optional optimization)
            if step % update_weights_every == 0:
                current_context = self.tokenizer.decode(input_ids[0])
    ...
    def predict_next_token_probabilities(self, context: str, top_k_tokens: int = 10) -> Dict:
        ...
        """
        # Compute ensemble weights for context
        ensemble_weights = self.compute_ensemble_weights(context)
        
        # Tokenize context and move to the device of the first expert
        input_ids = self.tokenizer.encode(context, return_tensors='pt')
        if self.experts:
            device = next(self.experts[0].parameters()).device
            input_ids = input_ids.to(device)
...
def load_cbtm_model(vectorizer_path: str, 
    ...
    for path in sorted(expert_dir_path.iterdir()):
        if path.is_dir():
            ...
            if (path / "checkpoint-421/pytorch_model.bin").exists() or (path / "checkpoint-421/model.safetensors").exists(): #TODO Update Later
